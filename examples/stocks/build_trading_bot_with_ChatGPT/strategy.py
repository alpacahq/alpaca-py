...
ALPACA_API_KEY = secret.API_KEY
ALPACA_SECRET_KEY = secret.API_SECRET
PAPER = secret.PAPER
trade_api_url = secret.trade_api_url
# setup trading clients
trade_client = TradingClient(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY, paper=PAPER, url_override=trade_api_url)
...
def fetch_bars(underlying_symbol, timeframe, days=90):
    today = datetime.now(local_tz).date()
    req = StockBarsRequest(
        symbol_or_symbols=[underlying_symbol],
        timeframe=TimeFrame(amount=1, unit=timeframe),  # specify timeframe
...
while True:
    ...
    # defensive fallback: if still closed we already logged and slept
    if not clock.is_open:
        continue
    
    # Fetch data
    df_main = fetch_bars(underlying_symbol, TIMEFRAME_MAIN, days=300)
    df_trend = fetch_bars(underlying_symbol, TIMEFRAME_TREND, days=MA_SLOW + 10)

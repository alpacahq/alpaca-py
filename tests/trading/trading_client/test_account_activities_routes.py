...
from alpaca.trading.client import TradingClient, PaginationType
...
def setup_reqmock_for_paginated_account_activities_response(reqmock: Mocker):
    ...
    reqmock.get(
        BaseURL.TRADING_PAPER.value + "/v2/account/activities",
        [{"text": resp_one}, {"text": resp_two}, {"text": """[]"""}],
    )


def test_get_activities_for_account_default_asserts(
    reqmock, trading_client: TradingClient
):
    setup_reqmock_for_paginated_account_activities_response(reqmock)

    result = trading_client.get_account_activities(GetAccountActivitiesRequest())
...
def test_get_activities_for_account_full_pagination(
    reqmock, trading_client: TradingClient
):
    setup_reqmock_for_paginated_account_activities_response(reqmock)

    result = trading_client.get_account_activities(
...
def test_get_activities_for_account_none_pagination(
    reqmock, trading_client: TradingClient
):
    setup_reqmock_for_paginated_account_activities_response(reqmock)
...
def test_get_account_activities_iterator_pagination(
    reqmock, trading_client: TradingClient
):
    setup_reqmock_for_paginated_account_activities_response(reqmock)

    generator = trading_client.get_account_activities(
...
def test_get_account_activities_validates_max_items(reqmock, client: TradingClient):
